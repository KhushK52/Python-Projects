    ...
    def nearest_bee(self):
        ...
        while current.entrance is not None:
            if not(current.is_hive):
                if current.bees and steps <= self.upper_bound and steps>=self.lower_bound:
...
class QueenAnt(ScubaThrower):  
    ...
    def action(self, gamestate):
        ...
        while location:
            if location.ant:
                if not(location.ant.double_damage):
                    location.ant.double()
                if location.ant.is_container and location.ant.ant_contained:
                    if not(location.ant.ant_contained.double_damage):
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        def slow_action(gamestate):
            if target.turns_slowed >0:
                target.turns_slowed -= 1
