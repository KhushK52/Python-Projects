...
def pick(paragraphs, select, k):
    ...
    ''
    """
    select_paras = [x for x in paragraphs if (select(x)==True)]
    if k+1 > (len(select_paras)):
        return ''
    else:
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    closest_word= word_list[differences.index(min_diff)]
    if min_diff <= limit:
        return closest_word
    else:
        return typed_word
...
def feline_fixes(typed, source, limit):
    ...
    5
    """
    def helper(t,s,count):
        if count>limit:
            return limit+1
        if not t and not s:       #empty list is falsey so not falsey will be truthy
            return count
        elif not t or not s:
            return helper(t[1:],s[1:],count+1)
        elif t[0]==s[0]:
            return helper(t[1:], s[1:], count)
        else:
            return helper(t[1:],s[1:],count+1)
...
def minimum_mewtations(typed, source, limit):
    ...
    # Recursive cases should go below here
        ...
        # BEGIN
        return 1 + min(add,substitute,remove)
        # END
...
def time_per_word(words, times_per_player):
    ...
    for time in times_per_player:
       word_time += [[(time[i+1]-time[i]) for i in range(len(time)-1)]]
...
def fastest_words(match):
    ...
    for w in word_indices:
        f_player = 0
        fastest_time = time(match,f_player,w)
        for i in player_indices:
            if time(match,i,w)<fastest_time:
                f_player = i
                fastest_time = time(match,f_player,w)       
        fastest_players += [f_player]
    f_words = []
    for p in player_indices:
        p_words = []
        for f in range((len(fastest_players))):
            if fastest_players[f]==p:
                p_words += [get_word(match,f)]
        f_words += [p_words]
    return f_words
